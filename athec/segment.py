...
def save_label(rgb, label, save_path):
    ...
    try:
        ...
        print(f"Image saved successfully to {save_path1}")
    ...
    try:
        ...
        print(f"Image saved successfully to {save_path2}")
    except Exception as e:
        print(f"Error: {e}")
    ...
    try:
        ...
        print(f"Image saved successfully to {save_path3}")
    except Exception as e:
        print(f"Error: {e}")
...
def tf_segment_quickshift(img,
    ...
    label, n_seg = skimage.measure.label(seg, background=99999999,
                                         return_num=True)  # by default 0-valued pixels are considered as background, so set background value to 999999
...
def attr_complexity_segment(img,
    ...
    h, w = seg.shape
    seg, n_seg = skimage.measure.label(seg, background=99999999,
                                       return_num=True)  # by default 0-valued pixels are considered as background, so set background value to 999999
    ...
    for segment_threshold in segment_thresholds:
        nseg_threshold = np.count_nonzero(
            counts >= (w * h * segment_threshold))  # number of segments larger than the threshold
    ...
    if len(sizes) < top_areas:
        import warnings
        warnings.warn(
            "The number of segments is fewer than the number of top segments requested. The remaining sizes are returned as 0.")
        sizes.extend(
            [0] * (top_areas - len(sizes)))  # if requested number of areas to return > the number of segments, add 0
